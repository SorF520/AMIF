...
class MSDTrainer(BaseTrainer):
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                acc, recall, precision, micro_f1 = get_four_metrics(true_labels, pred_labels)
                result = {
                    'eval_accuracy': acc,
                    'precision': precision,
                    'recall': recall,
    ...
    def _step(self, batch, mode="train"):
        input_ids, input_mask, segment_ids, caption_ids, caption_mask, caption_segment, img_mask, labels, images = batch
