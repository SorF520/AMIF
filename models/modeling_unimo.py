...
from models.XModules import SelfEncoder, CrossModalAlignment, GraphReasoning, SELayer, l2norm, SoftContrastiveLoss, AmbiguityLearning
...
class UnimoModel(nn.Module):
    def __init__(self, args, vision_config, text_config, add_pooling_layer=True, num_self_layer=1):
        ...
        # AGSA     模态内不一致性
        self.text_encoder = SelfEncoder(text_config, text_config.hidden_size, head=64, drop=0.0)
        self.vision_encoder = SelfEncoder(text_config, text_config.hidden_size, head=64, drop=0.0)

        # 模态间不一致性
        self.CrossModalAlignment = CrossModalAlignment(text_config, args)

        self.fc_sim_tranloc = nn.Linear(text_config.hidden_size, text_config.hidden_size)
        self.fc_sim_tranglo = nn.Linear(text_config.hidden_size, text_config.hidden_size)
        ...
        # 相似图推理
        self.SGR_module = nn.ModuleList([GraphReasoning(text_config.hidden_size) for i in range(args.SGR_step)])
        ...
        # self.fc_3 = nn.Linear(text_config.hidden_size, text_config.hidden_size)
        # self.fc_4 = nn.Linear(text_config.hidden_size, text_config.hidden_size)
        # 模糊性学习
        self.Ambiguity_module = AmbiguityLearning(weight_js=args.weight_js)

        # SE-block
        self.SE_fusion = SELayer(channel=3, reduction=16)

        self.text_pool = BertPooler(text_config)
        self.vision_pool = BertPooler(text_config)
        self.mix_pool = BertPooler(text_config)
    ...
    def forward(self,
        ...
        # AGSA
        final_text_output = self.text_encoder(text_encode_out)  # (32, 128, 768)
        final_vision_output = self.vision_encoder(vision_encode_out)  # (32, 50, 768)
        ...
        # mild_loss = self.mild_loss(torch.nn.functional.normalize(self.fc_3(text_cls_output)),
        #                      torch.nn.functional.normalize(self.fc_4(vision_cls_output)))

        # 局部相似性表征
        text_aware_image, cl_loss = self.CrossModalAlignment(text_output, vision_output)    # (32, 128, 768)

        sim_local = torch.pow(torch.sub(text_output, text_aware_image), 2)
        sim_local = l2norm(self.fc_sim_tranloc(sim_local), dim=-1)
        sim_local = self.fc_1(sim_local)

        # 全局相似性表征   （32, 768）
        sim_global = torch.pow(torch.sub(text_cls_output, vision_cls_output), 2)
        ...
        for module in self.SGR_module:
            sim_emb = module(sim_emb)  # (bsz, L+1, 768)

        # Fusion
        text_pooled_output = self.text_pool(final_text_output)  # (32, 768)
        image_pooled_output = self.vision_pool(final_vision_output)  # (32, 768)
        mix_pooled_output = self.mix_pool(sim_emb)  # (32, 768)

        # SE-block    modality-wise attention
        combine_representation = torch.cat((text_pooled_output.unsqueeze(1), mix_pooled_output.unsqueeze(1),
                                            image_pooled_output.unsqueeze(1)), dim=1)      # (bsz, 3, 768)

        # (bsz, 3, 768)     (bsz, 3)
        combine_output, channel_weight = self.SE_fusion(combine_representation)

        # 模糊性学习   JS散度 -> (bsz, 768) (bsz, 768) (bsz, 3)
        js_loss = self.Ambiguity_module(text_cls_output, vision_cls_output, channel_weight)

        # (bsz, 768)
        output = torch.sum(combine_output, dim=1)

        # # Pooler for classification
        # pooled_out_text = self.text_pooler(fusion_text_output)
